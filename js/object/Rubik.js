...
function SimpleCube(x, y, z, num, len, colors) {
  ...
  for (let i = 0; i < BasicParams.colors.length; i++) {
    let texture = new THREE.Texture(faces(BasicParams.colors[i]));
...
export default class Rubik {
  ...
  updateCurLocalAxisInWorld() {
    ...
    let matrix = this.group.matrixWorld; //魔方在世界坐标系的变换矩阵
   ...
   * 高度所占比例发生变化
   */
  resizeHeight(percent, transfromTag) {
    ...
    this.group.position.y = this.main.originHeight * (0.5 - percent / 2) * transfromTag
  }
}